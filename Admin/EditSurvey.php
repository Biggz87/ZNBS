	...
	<link rel="shortcut icon" href="media/image/favicon.ico" />
		...
		<div class="page-content">
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">
						<!-- BEGIN PORTLET-->   						<?php						$survey_id=$_GET['survey_id'];											if(isset($_POST['submitDownload'])){																																			// let's create some shortcuts// store the file										 $Title=$_POST['title'];  $closingdate=$_POST['Closingdate']; $dateposteddate=date('Y-m-d');   $status=$_POST['status'];      $SQL3 = "UPDATE `surveys` SET `Survey_title`='$Title',`status`='$status' WHERE `survey_id`='$survey_id'";     $result3= mysqli_query($conn,$SQL3);	   if($result3){     echo '<div class="alert alert-success"> The Survey has been Edited Successfully</div>'; echo "<script>window.location = 'Surveys.php?statusedit=Yes'</script>";    		}else {						 echo "<script>window.location = 'Surveys.php?statusedit=No</script>";								//}		}	}?> 
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Edit Survey</div>
								<div class="tools">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->																      <?php    $propertySQL="SELECT `survey_id`, `Survey_title`, `startdate`, `enddate`, `status` FROM `surveys` WHERE `survey_id`='$survey_id' ";											$results=mysqli_query($conn,$propertySQL);																						while($row5=mysqli_fetch_array($results)){											             ?>
								<form action="#" method="POST" class="form-horizontal">																  <div class="control-group">										<label class="control-label">Survey Title<span class="required">*</span></label>										<div class="controls">									                                                               											<input type="text"  name="title" value="<?php  echo $row5['Survey_title']; ?>" class="span6 m-wrap" required />										</div>									</div>
